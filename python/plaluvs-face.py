...
def start():
    ...
    image = cv2.imread('./data/sample3.jpg')
    ...
    for (i, rect) in enumerate(rects):
        ...
        rows,cols,_= out_face3.shape
        for i in range(rows):
            for j in range(cols):
                b,g,r = out_face3[i,j]
                if b==0 and g==0 and r==0:
                    out_face3[i,j] = [0,0,255]

        cv2.imshow("output1-beforefilter.jpg",out_face1)
        cv2.waitKey(0) 
        cv2.imshow("output2-beforefilter.jpg",out_face2)
        cv2.waitKey(0) 
        cv2.imshow("output3-beforfilter.jpg",out_face3)
        cv2.waitKey(0) 
        ...
        cv2.imshow("output1-afterfilter.jpg",out_face1fillter)
        cv2.waitKey(0) 
        cv2.imshow("output2-afterfilter.jpg",out_face2fillter)
        cv2.waitKey(0) 
        cv2.imshow("output3-afterfilter.jpg",out_face3fillter)
        cv2.waitKey(0) 
        ...
        for i in range(rows):
            for j in range(cols):
                b,g,r = out_face1Normal[i,j]
                if b<30 and g<30 and r<30:
                    mogong=mogong+1
                    out_face1Normal[i,j] = [0,0,255]
        print('/코 트러블%:', (count/mogong)*100)
        ...
        print('/왼쪽  볼 트러블%:', (count2/mogong2)*100)

        rows3,cols3,_3= out_face3Normal.shape
        mogong3=0
        for i in range(rows3):
            for j in range(cols3):
                b,g,r = out_face3Normal[i,j]
                if b<30 and g<30 and r<30:
                    mogong3=mogong3+1
                    out_face3Normal[i,j] = [0,0,255]
